    ...
    namespace algorithms
    {
        namespace kernel
        {
            struct Histogram
            {
                ...
                onlyMaster([&](uint32_t const, uint32_t const) { firstFrame = pb.getFirstFrame(superCellIdx); });

                /* loop over all particles in the frame */
                ForEachIdx<ParticleDomCfg> forEachParticle(workerIdx);

                forEachParticle([&](uint32_t const linearIdx, uint32_t const) { nppc[linearIdx] = 0u; });
