        ...
        namespace photonDetector
        {
            namespace accumulation
            {
                namespace acc
                {
                    ...
                    struct AmplitudeWithPolarization
                    {
                    public:
                        using ValueType = float_64;
                        //! type used to store detector cell values
                        using Type = pmacc::math::Vector<ValueType, DIM2>;
                         ...
                         */
                        HDINLINE AmplitudeWithPolarization(
                        ...
                        {
                            const ValueType weight = precisionCast<ValueType>(particle[weighting_]);
                            const float_X particleAngle = particle[polarizationAngle_];

                            // TODO: gloabal polarization angle defintion
                            ...
                            const ValueType valX = weight * math::sin(particleAngle);
                            const ValueType valY = weight * math::cos(particleAngle);
                ...
                public:
                    //! type used to store detector cell values
                    using Type = pmacc::math::Vector<float_64, DIM2>;
                    //! the value used to initialize (reset) detector storage
                    const Type initValue = Type::create(0.0);
                    using AccFunctorType = acc::AmplitudeWithPolarization<T_Species>;
                    ...
                    {
                        return "AmplitudeWithPolarization";
                    }
                     ...
                     */
                    HINLINE AccFunctorType operator()(
                        uint32_t const& currentStep,
                        DetectorParams const& detector,
                        DataSpace<simDim> const& simSize,
                        externalBeam::AxisSwap const& axisSwap) const
                    {
                        return AccFunctorType(currentStep, detector, simSize);
                    }
                };
