...
namespace picongpu
{
    namespace plugins
    {
        namespace photonDetector
        {
            ...
            class PhotonDetector : public plugins::multi::IInstance
            {
            public:
                ...
                struct Help : public plugins::multi::IHelp
                {
                     ...
                     *
                     * @returns shared pointer to the slave instance with the IInstance interface
                     */
                    ...
                    plugins::multi::Option<int32_t> detectorSizeX
                        = {"size.x", "detector size, in cells, in x direction (detector coordinate system)."};
                    ...
                    plugins::multi::Option<std::string> placement
                        = {"placement",
                           "Placement of the detector. Simulation box side behind which the detetector is placed. "
                    ...
                    std::string const prefix = T_Species::FrameType::getName() + "_" + T_DetectorConfig::getName()
                        + "_" + std::string("photonDetector");
                };
            ...
            private:
                ...
                // OpenPMD API doesn't fully support vectors.
                // Instead we create a separate mesh for each vector component.
                // Here we extract the type stored in a vector.
                // This is only rally needed when the accumulation policy uses vectors to store data.
                using SingleValueType = typename StoredType::type;

                using OpenPMDStoredType = typename openPMD::GetOpenPMDStoredType<SingleValueType>::type;
                using ParticleFilter = typename T_DetectorConfig::ParticleFilter::template apply<T_Species>::type;
                static constexpr uint32_t storedVectorSize = static_cast<uint32_t>(StoredType::dim);
                ...
                using BufferMaster = std::vector<StoredType>;
                BufferMaster detectorBufferMaster;
            ...
            public:
                    ...
                    , isMaster(reduce.hasResult(ReduceMethod()))
                {
                    ...
                    if(isMaster)
                    {
                        detectorBufferMaster.assign(
                            detectorSize.x() * detectorSize.y(),
                            AccumulationPolicy().initValue);
                    }
                }
            ...
            private:
                HINLINE void reduceDataOnMaster()
                {
                    ...
                    reduce(
                        ...
                        isMaster ? detectorBufferMaster.data() : nullptr,
                        detector->getHostDataBox().getPointer(),
                        detectorSize.productOfComponents(),
                        ReduceMethod());
                }

                HINLINE void writeOutputScalar(uint32_t currentStep)
                {
                    reduceDataOnMaster();

                    if(isMaster)
                    {
                        writer->openIteration(currentStep);
                        (*writer)(
                            currentStep,
                            reinterpret_cast<OpenPMDStoredType*>(detectorBufferMaster.data()),
                            detector->accumHostFunctor.getOpenPMDMeshSuffix(0));
                        writer->flush();
                        writer->closeIteration();
                        detectorBufferMaster.assign(
                            detectorSize.x() * detectorSize.y(),
                            AccumulationPolicy().initValue);
                    }
                ...
                HINLINE void writeOutputVector(uint32_t currentStep)
                {
                    reduceDataOnMaster();
                    ...
                    if(isMaster)
                    {
                        ...
                        const auto size = detectorBufferMaster.size();
                        std::vector<SingleValueType> component2d(size);

                        for(uint32_t component = 0; component < storedVectorSize; component++)
                        {
                            for(uint32_t ii = 0; ii < size; ii++)
                            {
                                component2d[ii] = detectorBufferMaster[ii][component];
                            }
                            (*writer)(
                                currentStep,
                                reinterpret_cast<OpenPMDStoredType*>(component2d.data()),
                                detector->accumHostFunctor.getOpenPMDMeshSuffix(component));
                ...
                {
                    // TODO: (c++17) make it constexpr if
                    if(storedVectorSize > 1u)
                        writeOutputVector(currentStep);
                    else
                        writeOutputScalar(currentStep);
                }
            ...
            public:
                 ...
                 *  definition.
                 */
                HINLINE void onParticleLeave(std::string const& speciesName_p, int32_t const direction) override
                {
                    ...
                    auto const mapper = mapperFactory(this->cellDescription_m);
                    ...
                    const auto detectParticle = detector->getDetectParticle(currentStep);
                    ...
                    (mapper.getGridDim(), numWorkers)(
                        ...
                        particles::filter::IUnary<ParticleFilter>{currentStep});
                }
            };
    ...
    namespace particles
    {
        namespace traits
        {
                ...
                plugins::photonDetector::PhotonDetector<T_DetectorConfig, T_UnspecifiedSpecies>>
            {
                ...
                using Filter = typename T_DetectorConfig::ParticleFilter;
                ...
                using EligibleForFilter =
                    typename particles::traits::SpeciesEligibleForSolver<T_Species, Filter>::type;
                ...
                using type = typename bmpl::
                    and_<typename bmpl::and_<EligibleForRayTracing, EligibleForAccumulation>, EligibleForFilter>;
            };
        } // namespace traits
    } // namespace particles
} // namespace picongpu
