...
namespace picongpu
{
    namespace plugins
    {
        namespace photonDetector
        {
            namespace detail
            {
                // TODO move this to pmacc::math
                //! Round a floating point value towards zero
                template<typename T>
                HDINLINE int roundTowardsZero(T const& val);

                template<>
                HDINLINE int roundTowardsZero<float>(float const& val)
                {
#if(CUPLA_DEVICE_COMPILE == 1) // we are on gpu
                    return ::__float2int_rz(val);
#endif
                    int ceilVal = math::ceil(math::abs(val));
                    return val < 0 ? -1 * ceilVal : ceilVal;
                }
                template<>
                HDINLINE int roundTowardsZero(double const& val)
                {
#if(CUPLA_DEVICE_COMPILE == 1) // we are on gpu
                    return ::__double2int_rz(val);
#endif
                    int ceilVal = math::ceil(math::abs(val));
                    return val < 0 ? -1 * ceilVal : ceilVal;
                }
                ...
                struct GetTargetCellIdx
                {
                        ...
                        DataSpace<DIM2>& targetIdx) const
                    {
...
#if SIMDIM == DIM2
                        const float3_X inCellPosition3D
                            = axisSwap_m.rotate(float3_X{particle[position_].x(), particle[position_].y(), 0.0_X});
#else
                        const float3_X inCellPosition3D = axisSwap_m.rotate(particle[position_]);
#endif
                        const float2_X inCellPosition{
                            math::fmod(inCellPosition3D.x() + 1.0_X, 1.0_X),
                            math::fmod(inCellPosition3D.y() + 1.0_X, 1.0_X)};
                        ...
                        cellIdxX += (globalIdx.x() - simSize_m.x() / 2 + inCellPosition.x()) * CELL_LENGTH_DET_X_DIR
                            / cellHeight;
                        targetIdx.x() += pmacc::math::float2int_rd(cellIdxX);
                        ...
                        cellIdxY += (globalIdx.y() - simSize_m.y() / 2 + inCellPosition.y()) * CELL_LENGTH_DET_Y_DIR
                            / cellWidth;
                        targetIdx.y() += pmacc::math::float2int_rd(cellIdxY);
            ...
            public:
                    ...
                    , accumHostFunctor(AccumPolicy())
                {
                    ...
                    // (usually 0)
                    buffer->getDeviceBuffer().setValue(accumHostFunctor.initValue);
                    anglePerCell_m.x() = atan(cellHeight / distance);
                    anglePerCell_m.y() = atan(cellWidth / distance);
                }
t
                /** clear detector
                 *
                ...
                HINLINE DetectParticle getDetectParticle(uint32_t currentStep) const
                {
                    ...
                    return DetectParticle(
                        ...
                        accumHostFunctor(currentStep, detParams, simSizeDet, axisSwap),
                        axisSwap);
                }
