    ...
    namespace plugins
    {
        namespace photonDetector
        {
            ...
            private:
                ...
                std::unique_ptr<::openPMD::Iteration> iteration_m;
            ...
            private:
                 ...
                 * @param iteration
                 */
                HINLINE ::openPMD::Mesh prepareMesh(std::string const& meshSuffix)
                {
                    ::openPMD::Mesh mesh = iteration_m->meshes[meshName_m + meshSuffix];
            ...
            public:
                HINLINE void openIteration(uint32_t const currentStep)
                {
                    iteration_m
                        = std::make_unique<::openPMD::Iteration>(openPMDSeries_m->writeIterations()[currentStep]);
                    // Write simulation meta data
                    openPMD::WriteMeta()(*openPMDSeries_m, *iteration_m, currentStep);
                }
                ...
                HINLINE void operator()(
                    ...
                    T_ValueType* pointer,
                    std::string const& meshSuffix = "")
                {
                    //                    // read & write access type is not possible when the iterations are stored in
                    //                    separate files switch(iterationEncoding_m)
                    //                    {
                    ...
                    //                        break;
                    //                    // there is also variableBased encoding on the API's dev branch, not sure
                    //                    what it is and when
                    //                    // would it appear in a release
                    //                    default:
                    //                        throw std::runtime_error(
                    //                            "PhotonDetectorWriter: Internal error. Unrecognized iteration
                    //                            encoding.");
                    //                    }

                    ::openPMD::Mesh mesh = prepareMesh(meshSuffix);
