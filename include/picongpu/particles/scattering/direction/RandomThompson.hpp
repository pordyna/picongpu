    ...
    namespace particles
    {
        namespace scattering
        {
            namespace direction
            {
                namespace acc
                {
                    // TODO: combine with random, reuse functions.
                    template<typename T_Rng_precision>
                    struct RandomThompson
                    {
                        using float_R = T_Rng_precision;

                        template<typename T_rng, typename T_Particle>
                        DINLINE void operator()(T_rng& rng, T_Particle& particle) const
                        {
                            ...
                            // azimuth in -pi to pi range (fast math optimization)
                            trigo_X azimuthAngle = static_cast<trigo_X>(
                                rng() * pmacc::math::Pi<float_R>::doubleValue - pmacc::math::Pi<float_R>::value);

                            float_R polarAngleCosine;
                            if(rng() < static_cast<float_R>(3.0 / 4.0))
                            {
                                polarAngleCosine = rng();
                            }
                            else
                            {
                                polarAngleCosine = pmacc::math::max(pmacc::math::max(rng(), rng()), rng());
                            }
                            if(rng() > 0.5)
                                polarAngleCosine *= static_cast<float_R>(-1.0);
                            trigo_X polarAngle = static_cast<trigo_X>(math::acos<float_64>(polarAngleCosine));
