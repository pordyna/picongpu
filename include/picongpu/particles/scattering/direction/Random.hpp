            ...
            namespace direction
            {
                namespace acc
                {
                    ...
                    private:
                        ...
                        // using -pi, +pi instead of 0, 2pi has something to do with precision and fast math
                        static constexpr float_X minAzimuth = -pmacc::math::Pi<float_X>::value;
                        static constexpr float_X maxAzimuth = pmacc::math::Pi<float_X>::value;
                    ...
                    public:
                        ...
                        DINLINE void operator()(T_rng& rng, T_Particle& particle) const
                        {
                            ...
                            float_X azimuthAngle = rng() * float_X(maxAzimuth - minAzimuth) + float_X(minAzimuth);
                            ...
                            if(std::is_same<float_X, float_32>::value && minPolar == 0. && maxPolar <= 1e-2)
                            {
                            ...
                            else
                            {
                                ...
                                const float_64 minPolarCos
                                    = (minPolar == 0.) ? float_64(1.) : math::cos(float_64(minPolar));
                                const float_64 maxPolarCos = math::cos(float_64(maxPolar));
                                // Note that cos(minPolar)>=cos(maxPolor) for minPolar<=maxPolar
                                polarAngle = math::acos<float_64>(rng() * (minPolarCos - maxPolarCos) + maxPolarCos);
                            }
