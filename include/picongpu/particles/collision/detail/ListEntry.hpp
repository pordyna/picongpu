...
namespace picongpu::particles::collision
{
    namespace detail
    {
        /** Index access to particles within a cell of an supercell.
         *
        ...
        public:
                ...
                T_NumParticlesArray& numParArray)
            {
                ...
                onlyMaster(
                    [&]()
                    {
                        ...
                        framePtr = (void**) allocMem<cellListChunkSize * framePtrBytes>(
                            worker,
                            numFrames * framePtrBytes,
                            deviceHeapHandle);
                ...
                forEachCell(
                    [&](uint32_t const cellIdx)
                    {
                        ...
                        constexpr uint32_t chunkSize = cellListChunkSize * frameSize * sizeof(uint32_t);
                        particleList[cellIdx] = (uint32_t*)
                            allocMem<chunkSize>(worker, sizeof(uint32_t) * numParsInCell, deviceHeapHandle);
                    });
            }

            /* Update cell particle index list
             *
             * Update the contiguous index list per cell with particle indies.
             */
            ...
            /** Creates an access to access particles within a cell
             *
        ...
        //! Cound number of particles per cell in the supercell
         ...
         *
         * Shuffle the given array with indicies.
         *
