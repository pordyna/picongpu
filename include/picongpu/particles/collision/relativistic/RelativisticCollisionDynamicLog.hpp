        ...
        namespace collision
        {
            namespace relativistic
            {
                namespace acc
                {
                    struct DynamicLog
                    {
                        DINLINE float_COLL operator()(Variables const& v) const
                        {
                            ...
                            const float_COLL factor1 = (v.charge0 * v.charge1)
                             ...
                             * developers the square in the original formula was a typo.
                             */
                            const float_COLL twoRadImpactParam = factor1 * factor2 * factor3;
                            ...
                            const float_COLL minImpactParam = pmacc::math::max(
                                HBAR * pmacc::math::Pi<float_COLL>::doubleValue
                                    / (2._COLL * math::sqrt(v.comsMomentum0Abs2)),
                                twoRadImpactParam);
                ...
                struct RelativisticCollisionDynamicLogImpl
                {
                    ...
                    struct apply
                    {
                        using type = RelativisticCollisionDynamicLog;
                    };
                    ...
                    using AccFunctorImpl = acc::RelativisticCollision<acc::DynamicLog>;
